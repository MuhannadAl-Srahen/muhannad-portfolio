...
export default function FlyingOrbs({ variant = 'default' }: FlyingOrbsProps) {
  ...
  return (
    <div className='absolute inset-0 z-0 pointer-events-none'>
      ...
      <div
        className='absolute bottom-1/3 right-1/3 w-64 h-64 bg-accent/12 rounded-full blur-3xl animate-pulse-slow'
        style={{ animationDelay: '5s' }}
      />
      <div
        className='absolute top-1/2 right-1/4 w-96 h-96 bg-primary/8 rounded-full blur-3xl animate-breathe'
        style={{ animationDelay: '2s' }}
      />
      <div
        className='absolute bottom-1/4 left-1/3 w-72 h-72 bg-accent/10 rounded-full blur-3xl animate-breathe'
